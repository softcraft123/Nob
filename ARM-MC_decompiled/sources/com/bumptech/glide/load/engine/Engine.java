...
public class Engine implements EngineJobListener, MemoryCache.ResourceRemovedListener, EngineResource.ResourceListener {
    ...
    public <R> LoadStatus load(GlideContext glideContext, Object obj, Key key, int i, int i2, Class<?> cls, Class<R> cls2, Priority priority, DiskCacheStrategy diskCacheStrategy, Map<Class<?>, Transformation<?>> map, boolean z, boolean z2, Options options, boolean z3, boolean z4, boolean z5, boolean z6, ResourceCallback resourceCallback, Executor executor) {
        ...
        synchronized (this) {
            boolean z7 = z3;
            EngineResource<?> loadFromMemory = loadFromMemory(buildKey, z7, logTime);
            if (loadFromMemory == null) {
                boolean z8 = z7;
                LoadStatus waitForExistingOrStartNewJob = waitForExistingOrStartNewJob(glideContext, obj, key, i, i2, cls, cls2, priority, diskCacheStrategy, map, z, z2, options, z8, z4, z5, z6, resourceCallback, executor, buildKey, logTime);
                return waitForExistingOrStartNewJob;
            }
            EngineResource<?> engineResource = loadFromMemory;
            resourceCallback.onResourceReady(engineResource, DataSource.MEMORY_CACHE, false);
            return null;
        }
    }
    ...
    private <R> LoadStatus waitForExistingOrStartNewJob(GlideContext glideContext, Object obj, Key key, int i, int i2, Class<?> cls, Class<R> cls2, Priority priority, DiskCacheStrategy diskCacheStrategy, Map<Class<?>, Transformation<?>> map, boolean z, boolean z2, Options options, boolean z3, boolean z4, boolean z5, boolean z6, ResourceCallback resourceCallback, Executor executor, EngineKey engineKey, long j) {
        ...
        EngineJob build = this.engineJobFactory.build(engineKey2, z3, z4, z5, z7);
        EngineKey engineKey3 = engineKey;
        DecodeJob<R> build2 = this.decodeJobFactory.build(glideContext, obj, engineKey3, key, i, i2, cls, cls2, priority, diskCacheStrategy, map, z, z2, z6, options, build);
        EngineKey engineKey4 = engineKey3;
        EngineJob engineJob2 = build;
        this.jobs.put(engineKey4, engineJob2);
        engineJob2.addCallback(resourceCallback2, executor2);
        engineJob2.start(build2);
        if (VERBOSE_IS_LOGGABLE) {
            logWithTimeAndKey("Started new load", j, engineKey4);
        }
        return new LoadStatus(resourceCallback2, engineJob2);
    }
