...
final class SQLiteMutationQueue implements MutationQueue {
    ...
    public /* synthetic */ void m448lambda$start$0$comgooglefirebasefirestorelocalSQLiteMutationQueue(Cursor cursor) {
        this.lastStreamToken = ByteString.copyFrom(cursor.getBlob(0));
    }

    private void loadNextBatchIdAcrossAllUsers() {
        ArrayList<String> arrayList = new ArrayList<>();
        this.db.query("SELECT uid FROM mutation_queues").forEach(new SQLiteMutationQueue$$ExternalSyntheticLambda8(arrayList));
        this.nextBatchId = 0;
        for (String str : arrayList) {
            this.db.query("SELECT MAX(batch_id) FROM mutations WHERE uid = ?").binding(str).forEach(new SQLiteMutationQueue$$ExternalSyntheticLambda9(this));
        }
        this.nextBatchId++;
    }
    ...
    public /* synthetic */ void m446lambda$loadNextBatchIdAcrossAllUsers$2$comgooglefirebasefirestorelocalSQLiteMutationQueue(Cursor cursor) {
    ...
    public MutationBatch lookupMutationBatch(int i) {
        return (MutationBatch) this.db.query("SELECT SUBSTR(mutations, 1, ?) FROM mutations WHERE uid = ? AND batch_id = ?").binding(1000000, this.uid, Integer.valueOf(i)).firstValue(new SQLiteMutationQueue$$ExternalSyntheticLambda2(this, i));
    }
    ...
    public /* synthetic */ MutationBatch m447lambda$lookupMutationBatch$3$comgooglefirebasefirestorelocalSQLiteMutationQueue(int i, Cursor cursor) {
        return decodeInlineMutationBatch(i, cursor.getBlob(0));
    }

    public MutationBatch getNextMutationBatchAfterBatchId(int i) {
        return (MutationBatch) this.db.query("SELECT batch_id, SUBSTR(mutations, 1, ?) FROM mutations WHERE uid = ? AND batch_id >= ? ORDER BY batch_id ASC LIMIT 1").binding(1000000, this.uid, Integer.valueOf(i + 1)).firstValue(new SQLiteMutationQueue$$ExternalSyntheticLambda10(this));
    }
    ...
    public /* synthetic */ MutationBatch m445lambda$getNextMutationBatchAfterBatchId$4$comgooglefirebasefirestorelocalSQLiteMutationQueue(Cursor cursor) {
    ...
    public List<MutationBatch> getAllMutationBatches() {
        ...
        this.db.query("SELECT batch_id, SUBSTR(mutations, 1, ?) FROM mutations WHERE uid = ? ORDER BY batch_id ASC").binding(1000000, this.uid).forEach(new SQLiteMutationQueue$$ExternalSyntheticLambda6(this, arrayList));
        return arrayList;
    }
    ...
    public /* synthetic */ void m441lambda$getAllMutationBatches$6$comgooglefirebasefirestorelocalSQLiteMutationQueue(List list, Cursor cursor) {
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingDocumentKey(DocumentKey documentKey) {
        ...
        ArrayList arrayList = new ArrayList();
        this.db.query("SELECT m.batch_id, SUBSTR(m.mutations, 1, ?) FROM document_mutations dm, mutations m WHERE dm.uid = ? AND dm.path = ? AND dm.uid = m.uid AND dm.batch_id = m.batch_id ORDER BY dm.batch_id").binding(1000000, this.uid, encode).forEach(new SQLiteMutationQueue$$ExternalSyntheticLambda0(this, arrayList));
        return arrayList;
    }
    ...
    public /* synthetic */ void m442lambda$getAllMutationBatchesAffectingDocumentKey$7$comgooglefirebasefirestorelocalSQLiteMutationQueue(List list, Cursor cursor) {
        list.add(decodeInlineMutationBatch(cursor.getInt(0), cursor.getBlob(1)));
    }
    ...
    public /* synthetic */ void m443lambda$getAllMutationBatchesAffectingDocumentKeys$8$comgooglefirebasefirestorelocalSQLiteMutationQueue(Set set, List list, Cursor cursor) {
    ...
    public List<MutationBatch> getAllMutationBatchesAffectingQuery(Query query) {
        ...
        ArrayList arrayList = new ArrayList();
        this.db.query("SELECT dm.batch_id, dm.path, SUBSTR(m.mutations, 1, ?) FROM document_mutations dm, mutations m WHERE dm.uid = ? AND dm.path >= ? AND dm.path < ? AND dm.uid = m.uid AND dm.batch_id = m.batch_id ORDER BY dm.batch_id").binding(1000000, this.uid, encode, prefixSuccessor).forEach(new SQLiteMutationQueue$$ExternalSyntheticLambda3(this, arrayList, path.length() + 1));
        return arrayList;
    }
    ...
    public /* synthetic */ void m444lambda$getAllMutationBatchesAffectingQuery$10$comgooglefirebasefirestorelocalSQLiteMutationQueue(List list, int i, Cursor cursor) {
    ...
    public void performConsistencyCheck() {
        if (isEmpty()) {
            ArrayList arrayList = new ArrayList();
            this.db.query("SELECT path FROM document_mutations WHERE uid = ?").binding(this.uid).forEach(new SQLiteMutationQueue$$ExternalSyntheticLambda7(arrayList));
