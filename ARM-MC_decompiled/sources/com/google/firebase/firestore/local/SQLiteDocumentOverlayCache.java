...
public class SQLiteDocumentOverlayCache implements DocumentOverlayCache {
    ...
    public Overlay getOverlay(DocumentKey documentKey) {
        return (Overlay) this.db.query("SELECT overlay_mutation, largest_batch_id FROM document_overlays WHERE uid = ? AND collection_path = ? AND document_id = ?").binding(this.uid, EncodedPath.encode((ResourcePath) documentKey.getPath().popLast()), documentKey.getPath().getLastSegment()).firstValue(new SQLiteDocumentOverlayCache$$ExternalSyntheticLambda4(this));
    }
    ...
    public /* synthetic */ Overlay m431lambda$getOverlay$0$comgooglefirebasefirestorelocalSQLiteDocumentOverlayCache(Cursor cursor) {
    ...
    private void processSingleCollection(Map<DocumentKey, Overlay> map, BackgroundQueue backgroundQueue, ResourcePath resourcePath, List<Object> list) {
        if (!list.isEmpty()) {
            ...
            while (longQuery.hasMoreSubqueries()) {
                longQuery.performNextSubquery().forEach(new SQLiteDocumentOverlayCache$$ExternalSyntheticLambda5(this, backgroundQueue, map));
            }
        }
    }
    ...
    public Map<DocumentKey, Overlay> getOverlays(ResourcePath resourcePath, int i) {
        HashMap hashMap = new HashMap();
        BackgroundQueue backgroundQueue = new BackgroundQueue();
        this.db.query("SELECT overlay_mutation, largest_batch_id FROM document_overlays WHERE uid = ? AND collection_path = ? AND largest_batch_id > ?").binding(this.uid, EncodedPath.encode(resourcePath), Integer.valueOf(i)).forEach(new SQLiteDocumentOverlayCache$$ExternalSyntheticLambda0(this, backgroundQueue, hashMap));
        backgroundQueue.drain();
        return hashMap;
    }
    ...
    public Map<DocumentKey, Overlay> getOverlays(String str, int i, int i2) {
        ...
        BackgroundQueue backgroundQueue = new BackgroundQueue();
        String str2 = str;
        this.db.query("SELECT overlay_mutation, largest_batch_id, collection_path, document_id  FROM document_overlays WHERE uid = ? AND collection_group = ? AND largest_batch_id > ? ORDER BY largest_batch_id, collection_path, document_id LIMIT ?").binding(this.uid, str2, Integer.valueOf(i), Integer.valueOf(i2)).forEach(new SQLiteDocumentOverlayCache$$ExternalSyntheticLambda2(this, iArr, strArr, strArr2, backgroundQueue, hashMap));
        if (strArr[0] == null) {
            return hashMap;
        }
        ...
        query.binding(str3, str2, str4, str4, strArr2[0], Integer.valueOf(iArr[0])).forEach(new SQLiteDocumentOverlayCache$$ExternalSyntheticLambda3(this, backgroundQueue, hashMap));
        backgroundQueue.drain();
        return hashMap;
    }
    ...
    public /* synthetic */ void m433lambda$getOverlays$3$comgooglefirebasefirestorelocalSQLiteDocumentOverlayCache(int[] iArr, String[] strArr, String[] strArr2, BackgroundQueue backgroundQueue, Map map, Cursor cursor) {
        iArr[0] = cursor.getInt(1);
        strArr[0] = cursor.getString(2);
        strArr2[0] = cursor.getString(3);
        m436lambda$processSingleCollection$1$comgooglefirebasefirestorelocalSQLiteDocumentOverlayCache(backgroundQueue, map, cursor);
    }

    /* access modifiers changed from: private */
    /* renamed from: processOverlaysInBackground */
    public void m436lambda$processSingleCollection$1$comgooglefirebasefirestorelocalSQLiteDocumentOverlayCache(BackgroundQueue backgroundQueue, Map<DocumentKey, Overlay> map, Cursor cursor) {
        SQLiteDocumentOverlayCache$$ExternalSyntheticLambda1 sQLiteDocumentOverlayCache$$ExternalSyntheticLambda1 = new SQLiteDocumentOverlayCache$$ExternalSyntheticLambda1(this, cursor.getBlob(0), cursor.getInt(1), map);
        if (!cursor.isFirst() || !cursor.isLast()) {
            backgroundQueue.submit(sQLiteDocumentOverlayCache$$ExternalSyntheticLambda1);
        } else {
            sQLiteDocumentOverlayCache$$ExternalSyntheticLambda1.run();
        }
    }
    ...
    public /* synthetic */ void m435lambda$processOverlaysInBackground$5$comgooglefirebasefirestorelocalSQLiteDocumentOverlayCache(byte[] bArr, int i, Map map) {
