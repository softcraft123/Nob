...
public final class MutationBatch {
    ...
    public Map<DocumentKey, Mutation> applyToLocalDocumentSet(Map<DocumentKey, OverlayedDocument> map, Set<DocumentKey> set) {
        ...
        for (DocumentKey next : getKeys()) {
            MutableDocument mutableDocument = (MutableDocument) map.get(next).getDocument();
            FieldMask applyToLocalView = applyToLocalView(mutableDocument, map.get(next).getMutatedFields());
            ...
            if (calculateOverlayMutation != null) {
                hashMap.put(next, calculateOverlayMutation);
            }
