...
public class Datastore {
    ...
    public /* synthetic */ List m478lambda$commit$0$comgooglefirebasefirestoreremoteDatastore(Task task) throws Exception {
    ...
    public /* synthetic */ Map m479lambda$runAggregateQuery$1$comgooglefirebasefirestoreremoteDatastore(HashMap hashMap, Task task) throws Exception {
        if (!task.isSuccessful()) {
            if ((task.getException() instanceof FirebaseFirestoreException) && ((FirebaseFirestoreException) task.getException()).getCode() == FirebaseFirestoreException.Code.UNAUTHENTICATED) {
                this.channel.invalidateToken();
            }
