...
public class NotInFilter extends FieldFilter {
    ...
    public boolean matches(Document document) {
        Value field;
        if (!Values.contains(getValue().getArrayValue(), Values.NULL_VALUE) && (field = document.getField(getField())) != null && !field.hasNullValue() && !Values.contains(getValue().getArrayValue(), field)) {
            return true;
        }
        return false;
    }
