...
final class CircularDrawingDelegate extends DrawingDelegate<CircularProgressIndicatorSpec> {
    ...
    public void fillIndicator(Canvas canvas, Paint paint, float f, float f2, int i) {
        if (f != f2) {
            ...
            float f6 = f3;
            canvas.drawArc(new RectF(-f5, -f5, f5, f5), f6, f4, false, paint);
            float f7 = f4;
            if (this.displayedCornerRadius > 0.0f && Math.abs(f7) < 360.0f) {
                paint.setStyle(Paint.Style.FILL);
                float f8 = f6;
                Paint paint2 = paint;
                drawRoundedEnd(canvas, paint2, this.displayedTrackThickness, this.displayedCornerRadius, f8);
                Canvas canvas2 = canvas;
                drawRoundedEnd(canvas2, paint2, this.displayedTrackThickness, this.displayedCornerRadius, f8 + f7);
            }
        }
    }
