...
public class ChipDrawable extends MaterialShapeDrawable implements TintAwareDrawable, Drawable.Callback, TextDrawableHelper.TextDrawableDelegate {
    ...
    public void draw(Canvas canvas) {
        ...
        if (!bounds.isEmpty() && getAlpha() != 0) {
            if (this.alpha < 255) {
                canvas2 = canvas;
                i = CanvasCompat.saveLayerAlpha(canvas2, (float) bounds.left, (float) bounds.top, (float) bounds.right, (float) bounds.bottom, this.alpha);
            } else {
                canvas2 = canvas;
                i = 0;
            }
    ...
    private void drawText(Canvas canvas, Rect rect) {
        if (this.text != null) {
            ...
            canvas2.drawText(charSequence2, 0, charSequence2.length(), this.pointF.x, this.pointF.y, this.textDrawableHelper.getTextPaint());
            if (z) {
                canvas2.restoreToCount(i);
            }
        }
