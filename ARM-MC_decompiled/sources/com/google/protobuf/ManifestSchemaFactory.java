    ...
    private static <T> Schema<T> newSchema(Class<T> cls, MessageInfo messageInfo) {
        if (!GeneratedMessageLite.class.isAssignableFrom(cls)) {
            ...
            if (allowExtensions(messageInfo2)) {
                return MessageSchema.newSchema(cls2, messageInfo2, NewInstanceSchemas.full(), ListFieldSchema.full(), SchemaUtil.unknownFieldSetFullSchema(), ExtensionSchemas.full(), MapFieldSchemas.full());
            }
            return MessageSchema.newSchema(cls2, messageInfo2, NewInstanceSchemas.full(), ListFieldSchema.full(), SchemaUtil.unknownFieldSetFullSchema(), (ExtensionSchema<?>) null, MapFieldSchemas.full());
