    ...
    private class CallCredentialsApplyingTransport extends ForwardingConnectionClientTransport {
        ...
        public ClientStream newStream(MethodDescriptor<?, ?> methodDescriptor, Metadata metadata, CallOptions callOptions, ClientStreamTracer[] clientStreamTracerArr) {
            ...
            if (credentials != null) {
                ...
                ClientStreamTracer[] clientStreamTracerArr2 = clientStreamTracerArr;
                MetadataApplierImpl metadataApplierImpl = new MetadataApplierImpl(this.delegate, methodDescriptor2, metadata, callOptions2, this.applierListener, clientStreamTracerArr2);
                if (this.pendingApplier.incrementAndGet() > 0) {
                    this.applierListener.onComplete();
                    return new FailingClientStream(this.shutdownStatus, clientStreamTracerArr2);
                }
                AnonymousClass2 r9 = new CallCredentials.RequestInfo() {
                ...
                try {
                    if (!(credentials instanceof InternalMayRequireSpecificExecutor) || !((InternalMayRequireSpecificExecutor) credentials).isSpecificExecutorRequired() || callOptions2.getExecutor() == null) {
                        executor = CallCredentialsApplyingTransportFactory.this.appExecutor;
                    } else {
                        executor = callOptions2.getExecutor();
                    }
                    credentials.applyRequestMetadata(r9, executor, metadataApplierImpl);
                } catch (Throwable th) {
            ...
            ClientStreamTracer[] clientStreamTracerArr3 = clientStreamTracerArr;
            if (this.pendingApplier.get() >= 0) {
                return new FailingClientStream(this.shutdownStatus, clientStreamTracerArr3);
            }
            return this.delegate.newStream(methodDescriptor3, metadata2, callOptions3, clientStreamTracerArr3);
        }
