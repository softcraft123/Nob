...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    private void startInternal(final ClientCall.Listener<RespT> listener, Metadata metadata) {
        ...
        if (effectiveDeadline == null || !effectiveDeadline.isExpired()) {
            ...
            Double valueOf = Double.valueOf(((double) effectiveDeadline.timeRemaining(TimeUnit.NANOSECONDS)) / d2);
            if (l == null) {
                d = 0.0d;
            } else {
                d = ((double) l.longValue()) / d2;
            }
            this.stream = new FailingClientStream(Status.DEADLINE_EXCEEDED.withDescription(String.format("ClientCall started after %s deadline was exceeded %.9f seconds ago. Name resolution delay %.9f seconds.", new Object[]{str, valueOf, Double.valueOf(d)})), clientStreamTracers);
        }
    ...
    private class DeadlineTimer implements Runnable {
        ...
        public void run() {
            ...
            sb.append(String.format(locale, "Name resolution delay %.9f seconds. ", new Object[]{Double.valueOf(d)}));
