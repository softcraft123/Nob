...
public abstract class FileHandle implements Closeable {
    ...
    public final void writeNoCloseCheck(long j, Buffer buffer, long j2) {
        long j3 = j2;
        SegmentedByteString.checkOffsetAndCount(buffer.size(), 0, j3);
        long j4 = j + j3;
        long j5 = j;
        while (j5 < j4) {
            Segment segment = buffer.head;
            Intrinsics.checkNotNull(segment);
            int min = (int) Math.min(j4 - j5, (long) (segment.limit - segment.pos));
            protectedWrite(j5, segment.data, segment.pos, min);
            segment.pos += min;
            long j6 = (long) min;
            j5 += j6;
