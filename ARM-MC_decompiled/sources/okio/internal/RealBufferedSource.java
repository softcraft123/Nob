...
public final class RealBufferedSource {
    ...
    public static final String commonReadUtf8LineStrict(okio.RealBufferedSource realBufferedSource, long j) {
        ...
        if (j >= 0) {
            ...
            okio.RealBufferedSource realBufferedSource2 = realBufferedSource;
            long indexOf = realBufferedSource2.indexOf((byte) 10, 0, j2);
            if (indexOf != -1) {
                return Buffer.readUtf8Line(realBufferedSource2.bufferField, indexOf);
            }
            if (j2 < Long.MAX_VALUE && realBufferedSource2.request(j2) && realBufferedSource2.bufferField.getByte(j2 - 1) == 13 && realBufferedSource2.request(j2 + 1) && realBufferedSource2.bufferField.getByte(j2) == 10) {
                return Buffer.readUtf8Line(realBufferedSource2.bufferField, j2);
            }
            Buffer buffer = new Buffer();
            realBufferedSource2.bufferField.copyTo(buffer, 0, Math.min((long) 32, realBufferedSource2.bufferField.size()));
            throw new EOFException("\\n not found: limit=" + Math.min(realBufferedSource2.bufferField.size(), j) + " content=" + buffer.readByteString().hex() + Typography.ellipsis);
        }
        ...
        if (realBufferedSource.closed) {
            ...
            while (j3 < j2) {
                ...
                if (indexOf == -1) {
                    ...
                    if (size >= j4 || realBufferedSource.source.read(realBufferedSource.bufferField, 8192) == -1) {
                        break;
                    }
                    j3 = Math.max(j3, size);
                    b = b2;
