...
public final class Buffer {
    ...
    public static final int selectPrefix(okio.Buffer buffer, Options options, boolean z) {
        ...
        int[] trie$okio = options2.getTrie$okio();
    ...
    public static final byte commonGet(okio.Buffer buffer, long j) {
        Intrinsics.checkNotNullParameter(buffer, "<this>");
        long j2 = j;
        SegmentedByteString.checkOffsetAndCount(buffer.size(), j2, 1);
        Segment segment = buffer.head;
        if (segment == null) {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:46:0x00ea, code lost:
        throw new java.lang.NumberFormatException(r1 + " but was 0x" + okio.SegmentedByteString.toHexString(r0.getByte(r17)));
     */
    ...
    public static final String commonReadUtf8LineStrict(okio.Buffer buffer, long j) {
        ...
        if (j >= 0) {
            ...
            long j3 = j2;
            long indexOf = buffer.indexOf((byte) 10, 0, j3);
            if (indexOf != -1) {
                return readUtf8Line(buffer, indexOf);
            }
            ...
            buffer3.copyTo(buffer2, 0, Math.min((long) 32, buffer.size()));
        ...
        if (byteString2.size() > 0) {
            ...
            if (j3 >= 0) {
                ...
                if (buffer2.size() - j3 < j3) {
                    ...
                    long size2 = (buffer2.size() - ((long) size)) + 1;
                    while (j2 < size2) {
                        byte[] bArr = segment.data;
                ...
                long size4 = (buffer2.size() - ((long) size3)) + 1;
                while (j2 < size4) {
                    byte[] bArr2 = segment.data;
                    long j6 = size4;
