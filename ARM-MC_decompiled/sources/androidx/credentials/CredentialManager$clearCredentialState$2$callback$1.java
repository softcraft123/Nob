...
public final class CredentialManager$clearCredentialState$2$callback$1 implements CredentialManagerCallback<Void, ClearCredentialException> {
    ...
    public void onResult(Void voidR) {
        if (this.$continuation.isActive()) {
            ...
            this.$continuation.resumeWith(Result.m516constructorimpl(Unit.INSTANCE));
        }
    }
    ...
    public void onError(ClearCredentialException clearCredentialException) {
        ...
        if (this.$continuation.isActive()) {
            Result.Companion companion = Result.Companion;
            this.$continuation.resumeWith(Result.m516constructorimpl(ResultKt.createFailure(clearCredentialException)));
        }
    }
}
